...
static void increase_priority() {
    ...
    if (priority != old_priority) {
        if (verbose) {
            fprintf(stderr, "Priority changed to %d\n", priority);
        }
    } else {
        if (verbose) {
            fprintf(stderr, "Warning: Cannot change priority. Run as root or renice for best results.\n");
        }
    }
}
...
void limit_process(pid_t pid, double limit, int include_children, float minimum_cpu_usage, int chunk_size) {
    ...
    if (verbose) {
        fprintf(stderr, "Members in the process group owned by %d: %d\n", pgroup.target_pid, pgroup.proclist->count);
    }
    ...
    if (command_mode) {
        ...
        if (child < 0) {
            ...
            if (limiter < 0) {
                ...
                //limiter code
                if (verbose) {
                    fprintf(stderr, "Limiting process %d\n",child);
                }
